...
def force_numeric(df, column):
    """Forcefully converts a column to numeric, removing all non-digit characters."""
    if column not in df.columns:
        return pd.Series([0.0] * len(df))
    # Regex keeps only numbers and decimal points
    return pd.to_numeric(df[column].astype(str).str.replace(r'[^\d.]', '', regex=True), errors='coerce').fillna(0.0)
...
def ai_categorize_hybrid(df, desc_col):
    ...
    prompt = f"Categorize into [Food, Investment, Shopping, Rent, Salary, Sports/Hobbies, Bills, Misc]. Return ONLY JSON object with key 'categories'. Transactions: {descriptions}"
    ...
    try:
        ...
        for i, desc in enumerate(descriptions):
            ...
            final_categories.append(matched if matched else (ai_suggestions[i] if i < len(ai_suggestions) else "Misc"))
...
if uploaded_file:
    ...
    # --- CRITICAL: PRE-CHART NUMERIC CLEANING ---
    df[debit_col] = force_numeric(df, debit_col)
    df[credit_col] = force_numeric(df, credit_col)
    ...
    # Force numeric conversion ON THE EDITED DATA before charting
    edited_df[debit_col] = pd.to_numeric(edited_df[debit_col], errors='coerce').fillna(0)
    
    dash1, dash2 = st.columns(2)
    with dash1:
        st.subheader("Spending by Category")
        # Filter rows where spending > 0
        spend_data = edited_df[edited_df[debit_col] > 0]
        if not spend_data.empty:
            # Grouping and summing
            chart_ready = spend_data.groupby("Category")[debit_col].sum()
            st.bar_chart(chart_ready)
        else:
            st.info("No debit transactions found to display in chart.")
